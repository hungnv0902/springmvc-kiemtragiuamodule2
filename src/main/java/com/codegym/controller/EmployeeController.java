...
public class EmployeeController {
    ...
    public ModelAndView showEmployee(@PageableDefault(size = 2) Pageable pageable) {
        ...
        public ModelAndView saveEmployee(@ModelAttribute("employeeForm") EmployeeForm employeeForm, BindingResult result, HttpServletRequest servletRequest) {
            ...
            Employee productObject = new Employee(employeeForm.getName(), employeeForm.getBirthDate(), employeeForm.getDepartment(),employeeForm.getAddress(),fileName,employeeForm.getSalary());
    ...
    public ModelAndView showEditForm(@PathVariable Long id){
        ...
        if(employee != null) {
            EmployeeForm employeeForm = new EmployeeForm(employee.getId(),employee.getName(),employee.getBirthDate(),employee.getDepartment(),employee.getAddress(),null,employee.getSalary());
    ...
    public ModelAndView editEmployee(@ModelAttribute("employeeForm") EmployeeForm employeeForm, BindingResult result, Pageable pageable) {
        ...
        // tao doi tuong de luu vao db
        if(fileName != "") {
            Employee employeeObject = new Employee(employeeForm.getId(), employeeForm.getName(), employeeForm.getBirthDate(), employeeForm.getDepartment(), employeeForm.getAddress(), fileName, employeeForm.getSalary());
            employeeService.save(employeeObject);
        } else {
            Employee employeeObject = new Employee(employeeForm.getId(), employeeForm.getName(), employeeForm.getBirthDate(), employeeForm.getDepartment(), employeeForm.getAddress(),employee.getAvatar(), employeeForm.getSalary());
            employeeService.save(employeeObject);
        }
        // luu vao db
